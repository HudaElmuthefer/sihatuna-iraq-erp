...
-- ============================================================================
-- هذي الجداول ما عندها حالياً أي بحث أو فلترة من جهة السيرفر لأي حقل —
-- فمافيه حقل معيّن نرقّيه اليوم (كان بيصير تخميناً). بدل هذا، نضيف فهرس
-- GIN عام على عمود data كامل لكل جدول منها. هذا يجهّز البنية التحتية
-- لتتحمّل أي طلب فلترة/بحث تضيفينه مستقبلاً على أي حقل، بسرعة معقولة،
-- من اليوم الأول اللي تحتاجينه فيه — بدون انتظار migration جديد وقتها.
--
-- لما يطلب مستخدم فعلياً فلترة بحقل معيّن بموديول معيّن (مثلاً فلترة
-- الإجازات المرضية بنوع الإجازة)، بهذيك اللحظة نرقّي ذاك الحقل تحديداً
-- لعمود حقيقي مفهرس (نفس أسلوب patients/doctors اليوم) — بيبقى أسرع من
-- الفهرس العام هذا، لكن الفهرس العام يضمن أداء معقول بالانتظار.
--
