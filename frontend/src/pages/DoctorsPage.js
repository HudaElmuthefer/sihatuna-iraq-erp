...
export default function DoctorsPage() {
  ...
  const { lang, addToast, doctors, setDoctors, departments, syncToServer, hospitals, multiHospitalEnabled, filterByViewingHospital } = useApp();
  const tr = useT(lang);
  ...
  const filtered = doctors.filter(d => {
    ...
    const matchQ = (d.name || '').toLowerCase().includes(q) ||
      ...
      (d.specializationEn || d.spec || '').toLowerCase().includes(q);
        ...
        <div className="doctors-grid">
          {pageItems.map(doc => (
            <div key={doc.id} className="doctor-card">
              <div className="doctor-card-top" style={{ background: `linear-gradient(135deg, ${doc.color}22, ${doc.color}11)` }}>
