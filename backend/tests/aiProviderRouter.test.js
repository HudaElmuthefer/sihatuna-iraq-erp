...
describe('getSettings / getMode', () => {
  test('لا صف محفوظ إطلاقاً: كل الميزات تعود للافتراضي (online) — يطابق السلوك القديم قبل هذه الميزة', async () => {
    ...
    expect(settings).toEqual({ invoiceReader: DEFAULT_MODE, drugInteractions: DEFAULT_MODE, prescriptionReader: DEFAULT_MODE, aiDiagnosis: DEFAULT_MODE, dosageValidation: DEFAULT_MODE, allergyCheck: DEFAULT_MODE, billingAnomaly: DEFAULT_MODE, inventoryPrediction: DEFAULT_MODE });
  });
...
describe('setSettings', () => {
  test('تحديث جزئي: يدمج مع القيم الحالية، لا يمحوها', async () => {
    ...
    expect(result).toEqual({ invoiceReader: 'offline', drugInteractions: 'bot', prescriptionReader: 'bot', aiDiagnosis: 'online', dosageValidation: 'online', allergyCheck: 'online', billingAnomaly: 'online', inventoryPrediction: 'online' });
