...
// exist") لا يظهر إلا لحظة أول حفظ فعلي من مستخدم حقيقي. هذي الموديولات
...
const CASES = [
  ...
  ['promotions', () => ({ name: 'موظف اختبار الترفيع' })],
  ['allowances', () => ({ name: 'بدل اختبار', amount: 50000 })],
...
describe('فحص سلامة الموديولات — حماية من تكرار خطأ indexedColumns', () => {
  ...
  // كل مستوى يحتاج معرّف حقيقي من المستوى اللي فوقه (نفس مبدأ dossiers أعلاه)
  ...
  // هذا يغطي تحديداً خطأ ROLLBACK الحرج اللي أصلحناه سابقاً (كان يقدر يُسقط
  ...
  test('DELETE ثم POST /api/recycle-bin/:id/restore — دورة الحذف والاسترجاع كاملة', async () => {
    ...
    // اتأكد فعلاً انحذف من الجدول الأصلي (مو موجود بعد)
