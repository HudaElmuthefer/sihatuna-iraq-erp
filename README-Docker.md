...
|---|---|---|
| `nginx` | نقطة الدخول الوحيدة — يقدّم الفرونت إند مباشرة، يوجّه `/api` و`/uploads` لـ`app` داخلياً (راجع `nginx/nginx.conf`) | **80** |
...
بدل التثبيت المباشر. Nginx هنا ليس لموازنة حمل بين نسخ متعددة (لا يوجد
...
VPN/WireGuard لا HTTPS عام) — راجع التعليق أسفل `nginx/nginx.conf` لخطوات
إضافة SSL لاحقاً.
...
عبّ القيم الحقيقية بملف `.env` الجديد (بجذر المشروع، بجانب
...
> أي قيمة تلقائياً. لا تلتزم (commit) بملف `.env` الحقيقي لـ git أبداً —
...
docker compose down -v     # ⚠️ يحذف كل شيء بما فيها البيانات الدائمة — لا تستخدمه إلا لو تريد البدء من الصفر فعلاً
```
...
VPN/WireGuard لا HTTPS عام، فلا توجد شهادة SSL بعد. لكن `nginx/nginx.conf`
مبني من البداية بحيث إضافة SSL لاحقاً **تغيير صغير إضافي، لا إعادة كتابة**
— هذه الخطوات الكاملة وقتها.

### 1. احصل على شهادة

اختر حسب سيناريو النشر الفعلي وقتها:

- **دومين حقيقي، وصول عام** (Let's Encrypt، مجاني): استخدم `certbot`
  ...
  Let's Encrypt تنتهي صلاحيتها كل 90 يوم — تحتاج تجديداً دورياً
...
### 2. اربط مجلد الشهادات بخدمة nginx (`docker-compose.yml`)

أضف المنفذ 443 وvolume الشهادات لخدمة `nginx`:
...
### 3. عدّل `nginx/nginx.conf`

استبدل `server { listen 80; ... }` الحالي بسيرفرين: واحد يحوّل كل حركة
HTTP لـHTTPS تلقائياً، وثانٍ يخدم HTTPS فعلياً بنفس `location{}` الموجودة
حالياً حرفياً (انسخها كما هي، فقط أضف `server{}` جديد):
...
```nginx
  ...
  server {
    ...
    # انسخها حرفياً هنا (root، index، location /api/، location /uploads/،
    # location ~* \.(js|css|...)، location /).
  }
```

### 4. فعّل كوكي `secure` (`.env`)
...
فعلياً. لا تفعّله قبل التأكد إن HTTPS يعمل فعلياً، وإلا تسجيل الدخول يفشل
بصمت.)
...
افتح `https://<عنوان السيرفر>` بالمتصفح — يجب أن يعمل تسجيل الدخول
...
المطوّر) أو بـCI (`.github/workflows/test.yml`، بيئة منفصلة كاملة بحزمها
...
**لا — يبقيان طريقة تشغيل صالحة ومستقلة، ولم يُعدَّلا بهذه المرحلة.**

التوصية: اعتبر Docker (`docker compose up -d`) الطريقة **الموصى بها** لأي
...
- الجهاز لا يدعم Docker (موارد ضعيفة، أو قيود شبكة/صلاحيات مؤسسية تمنع
  تثبيته)
- تريد تجربة/تطوير سريعاً بدون بناء صورة Docker كاملة بكل تعديل بسيط
...
إطلاقاً بهذه المرحلة) — الفرق الوحيد هو طريقة خدمة الفرونت إند (خادم
React التطويري محلياً مقابل نسخة إنتاج ثابتة بـDocker، راجع
