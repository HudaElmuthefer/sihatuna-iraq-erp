...
// إطلاقاً، ممكن تكتب اسماً غير موجود أصلاً بالنظام. (2) كل الوثائق تُخزَّن
// بذاكرة المتصفح المؤقتة فقط (useState محلي) — تُفقَد بمجرد تحديث الصفحة.
// (3) الملف المرفق نفسه كان مجرد معاينة محلية (blob URL) ما يُرفَع لأي مكان.
// الآن: تختار من قائمة الموظفين الحقيقيين الفعليين، والوثائق تُحفَظ وتُرفَع
...
export default function DossiersTab({ lang }) {
  ...
  // الامتداد) لو لم تكتب عنواناً مسبقاً — تستطيع تعديله بعدها عادي.
  const handleFile = (e) => {
