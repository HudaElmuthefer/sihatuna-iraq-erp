...
export function AppProvider({ children }) {
  ...
  const [medNotifSource, setMedNotifSource] = useState({ admissions: [], orders: [], administrations: [], promotions: [], allowances: [] });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([admissions, orders, administrations, promotions, allowances]) => {
        if (cancelled) return;
        setMedNotifSource({
