...
// بدل ما يبقى عملية شغّالة بالخلفية بلا توقف — هذا أبسط وأوثق على ويندوز.
//
// المنطق الأهم: **تنبيه واحد فقط عند تغيّر الحالة**، مو رسالة كل فحص لو
// الخلل مستمر لساعات — غير هذا كان راح تغرقين برسائل متكررة لنفس المشكلة.
// يُبلَّغ أيضاً عند "التعافي" (رجوع النظام لطبيعته) حتى تعرفين متى انحلّت
// المشكلة بدون ما تراقبين يدوياً.
//
...
async function main() {
  ...
  devLog(`[${now}] فحص: ${currentStatus}${result.details?.database ? ` | قاعدة البيانات: ${result.details.database}` : ''}`);
