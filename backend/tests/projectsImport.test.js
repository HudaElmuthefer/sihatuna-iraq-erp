...
beforeAll(async () => {
  ...
  const probe = await request(app).get('/api/projects').set('Authorization', `Bearer ${token}`);
  assertPgAvailable(probe, 'استيراد المشاريع');
});
...
describe('استيراد مشروع من Excel بدون أعمدة spent/progress/milestones', () => {
  test('الحقول الرقمية الناقصة تُملأ بصفر تلقائياً بدل ما تبقى فاضية', async () => {
    ...
    expect(imported.completedMilestones).toBe(0);
  });
});
