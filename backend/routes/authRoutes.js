...
router.post('/auth/login', loginLimiter, (req, res) => {
  ...
  // لنظام داخلي متل هذا (ما فيه حالة استخدام مشروعة لفتح رابط من موقع خارجي
  ...
  // بينهما تماماً ويكسر تسجيل الدخول — لازم إرجاعه لـ'lax' بهذي الحالة تحديداً.
  res.cookie('auth_token', token, {
    httpOnly: true,
    secure: process.env.USE_HTTPS === 'true', // مرتبط بـ HTTPS الفعلي (متغيّر مستقل)، مو بوضع production بشكل عام — نظامك يشتغل حالياً عبر HTTP على شبكة محلية، فيبقى false افتراضياً حتى لو NODE_ENV=production. فعّليه فقط لو نصّبتِ شهادة SSL حقيقية.
    sameSite: 'strict',
