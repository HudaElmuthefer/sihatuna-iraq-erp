...
// الأعلى الذي يُزيل BarcodeTab كاملاً) قبل أن ينتهي تشغيل الكاميرا فعلياً،
// React يزيل <div id="barcode-reader-hr"> (وعنصر الفيديو بداخله) من DOM
// بينما وعد .play() الداخلي لا يزال معلّقاً — المتصفح يرفضه كـunhandled rejection
...
export default function BarcodeTab({ lang }) {
  ...
  useEffect(() => {
    ...
    // الورق مع الذي يظهر هنا لاحقاً، حتى لو تغيّرت طريقة التوليد مستقبلاً.
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      startPromise = scanner.start(
        ...
        // ── إصلاح: لو صار تبديل التبويب (cancelled=true) قبل أن ينتهي start()
        // فعلياً، الكاميرا كانت تبقى تعمل — لأن cleanup سبق واستدعى
        // stop() ومكتبة html5-qrcode ترفض إيقاف ماسح لا يزال بمرحلة "بدء التشغيل"
        // (يرمي خطأ يُبتلَع بصمت بالـ catch). الآن لو صار الإلغاء أثناء
        // الانتظار، نوقف الكاميرا فوراً بمجرد أن ينتهي start() فعلياً، ولا
